...
static IdSet static_analysis(AST *ast_, bool in_object, const IdSet &vars)
{
    ...
    case AST_FUNCTION: {
        ...
        for (const auto &p : ast->params) {
            if (params.find(p.id) != params.end()) {
                ...
                jsonnet_throw_static(StaticError(ast_->location, msg));
            }
            params.insert(p.id);
    ...
    case AST_IN_SUPER: {
        ...
        if (!in_object)
            jsonnet_throw_static(StaticError(ast_->location, "Can't use super outside of an object."));
    ...
    case AST_SELF: {
        ...
        if (!in_object)
            jsonnet_throw_static(StaticError(ast_->location, "Can't use self outside of an object."));
    } break;
    ...
    case AST_VAR: {
        ...
        if (vars.find(ast->id) == vars.end()) {
            jsonnet_throw_static(StaticError(ast->location, "Unknown variable: " + encode_utf8(ast->id->name)));
        }
        r.insert(ast->id);
    } break;
    default:
